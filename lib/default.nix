...
in
{
  ...
  #     escapeAttrName =
  #       name: if builtins.match "[a-zA-Z_][a-zA-Z0-9_'-]*" name != null then name else "\"${name}\"";
  #
  #     # Recursive function to build nested attribute set
  #     buildNested =
  #       remaining:
  #       if builtins.length remaining == 2 then
  #         {
  ...
  #         };
  #   in
  #   if builtins.length list == 1 then { ${builtins.head list} = true; } else buildNested list;
}
